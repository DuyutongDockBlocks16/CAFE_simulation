...
class MirobotController:
    ...
    def calculate_car_trajectory(self, target_pos, target_yaw):
        """为前轮转向车辆计算合适的轨迹"""
        pos = self.data.xpos[self.robot_1_rover_id][:2]
        quat = self.data.xquat[self.robot_1_rover_id]
        current_yaw = self.quat_to_yaw(quat)
        
        # 🎯 计算接近角度
        direct_heading = np.arctan2(target_pos[1] - pos[1], target_pos[0] - pos[0])
        
        # 🎯 如果目标朝向与接近方向差异很大，需要规划中间点
        yaw_diff = abs((target_yaw - direct_heading + np.pi) % (2 * np.pi) - np.pi)
        
        if yaw_diff > np.pi/3:  # 60度以上差异
            # 规划一个中间点，使车辆能够合理到达目标
            distance = np.linalg.norm(target_pos - pos)
            offset_dist = min(1.0, distance * 0.3)  # 偏移距离
            
            # 在目标朝向的反方向创建中间点
            intermediate_pos = target_pos - offset_dist * np.array([np.cos(target_yaw), np.sin(target_yaw)])
            return intermediate_pos, direct_heading  # 先到中间点
        else:
            return target_pos, target_yaw  # 直接到目标
    ...
    def pid_origin_position_to_picking_position(self,
        ...
        # 🎯 决定前进还是倒车
        if enable_reverse and forward_projection < -0.1:  # 目标在车辆后方
            # 使用倒车模式
            drive_sign = -1
            # 倒车时，朝向目标的相反方向
            target_heading = (np.arctan2(direction[1], direction[0]) + np.pi) % (2 * np.pi)
            # print(f"🔄 Reverse mode: target behind vehicle (projection: {forward_projection:.2f})")
        else:
            # 使用前进模式
            drive_sign = 1
            target_heading = np.arctan2(direction[1], direction[0])

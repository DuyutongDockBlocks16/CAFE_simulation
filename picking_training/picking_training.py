...
gym.register(
    id="SecondRobotPickingMuJoCoEnv-v0",
...
def picking_model_training(env, load_model_path=None):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"../logs/picking_episode_data_{timestamp}.jsonl"
    ...
    for i in range(num_iterations):
        ...
        if load_model_path:
            model_name = f"picking_model_continued_{current_total_steps // 1000}K"
        else:
            model_name = f"picking_model_{current_total_steps // 1000}K"
    ...
    if load_model_path:
        final_model_name = f"picking_model_continued_{final_total_steps // 1000}K_final"
    else:
        final_model_name = f"picking_model_{final_total_steps // 1000}K_final"
...
def picking_model_training_parallel(load_model_path=None, num_envs=8):
    ...
    if load_model_path is not None:
        ...
        model.tensorboard_log = f"./ppo_logs/picking_continued_parallel_{timestamp}/"
    ...
    if load_model_path:
        final_model_name = f"final_picking_model_continued_{final_total_steps // 1000}K_{timestamp}"
    else:
        final_model_name = f"final_picking_model_{final_total_steps // 1000}K_{timestamp}"

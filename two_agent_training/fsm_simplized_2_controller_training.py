...
def driver_model_training_parallel(load_model_path=None, num_envs=8):
    ...
    if load_model_path is not None:
        ...
        model = MaskablePPO(
            ...
            device='cpu',
                    learning_rate=3e-4,     
                    n_steps=8192,           # Adjust to suitable value for parallel environments
                    batch_size=256,          
                    n_epochs=8,            
                    ent_coef=0.02,          
                    clip_range=0.15,   
...
def driver_model_test_single_episode(env):
    ...
    while True:
        ...
        if terminated or truncated:
            ...
            print(f"  Termination reason: {'Success' if terminated else 'Timeout'}")
            print(f"  Final reward: {reward:.3f}")
            break
        
        if step_count > 10000:  # Prevent infinite loop
            print("⚠️ Maximum step limit reached")
            break

...
class CollabFsmHybridMuJoCoEnv(gym.Env):
    ...
    def __init__(self, action_repeat=10):
        super().__init__()
    ...
    def get_action_mask(self):
        ...
        if self.agent_robot == AgentRobot.ROBOT2:
            ...
            allowed = self.ALLOWED_ACTIONS[self.third_robot_status]
        masks[allowed] = True
        return masks
    ...
    def _original_step(self, action):
        ...
        if self.shared_state["current_object_index"] >= len(self.object_joint_ids) \
            and ( self.third_robot_status == RLRobotFiniteState.WAIT_FOR_FINISH or self.third_robot_status == RLRobotFiniteState.IDLE ) \
            and (self.second_robot_status == RLRobotFiniteState.WAIT_FOR_FINISH or self.second_robot_status == RLRobotFiniteState.IDLE):
        ...
        if self._check_robot_3_forbidden_collision():
            # print("Robot collision with forbidden area detected! Terminating episode.")
            reward -= 0.001
            self.check_robot_3_forbidden_collision_counter += 1
    ...
    def _reward_function_robot_2(self, action):
        ...
        # print(self.shared_state["current_object_index"])

        # print(action)
        # print(self.third_robot_status)
        action_switch = False
        ...
        if self.agent_robot == AgentRobot.ROBOT2:
            robot_status = self.second_robot_status
        elif self.agent_robot == AgentRobot.ROBOT3:
            robot_status = self.third_robot_status
        # print(robot_status)
    
        if robot_status == RLRobotFiniteState.IDLE:
        ...
        elif robot_status == RLRobotFiniteState.NAVIGATE_TO_PICKING_POSITION and \
        ...
        elif robot_status == RLRobotFiniteState.MOVING_TO_ORIGIN_POSITION:
            if action == 0:
                self._brake_robot2()
            elif action == 8:
        ...
        elif robot_status == RLRobotFiniteState.NAVIGATE_TO_PICKING_POSITION:
            ...
            if action == 0:
                ...
                if self.agent_robot == AgentRobot.ROBOT3:
                    action_switch = self._predict_navigation_action_robot_2(self.robot_3_target_position_x_y, Direction.BACKWARD, 0.2)
                if action_switch:
                    if self.agent_robot == AgentRobot.ROBOT2:
                        self.second_robot_status = RLRobotFiniteState.PICKING_OBJECT
                    elif self.agent_robot == AgentRobot.ROBOT3:
        ...
        elif robot_status == RLRobotFiniteState.PICKING_OBJECT:
        ...
        elif robot_status == RLRobotFiniteState.MAKE_DECISION_ON_PLACING_POSITION:
            if action == 0:
                self._brake_robot2()
            elif action == 4:
                if self.agent_robot == AgentRobot.ROBOT2:
        ...
        elif robot_status == RLRobotFiniteState.NAVIGATE_TO_PLACING_POSITION_1:
            ...
            if action == 0:
                ...
                if self.agent_robot == AgentRobot.ROBOT3:
                    ...
                    action_switch = self._predict_navigation_action_robot_2(self.robot_3_target_position_x_y, Direction.FORWARD, 0.4)
                if action_switch:
                    if self.agent_robot == AgentRobot.ROBOT2:
        ...
        elif robot_status == RLRobotFiniteState.NAVIGATE_TO_PLACING_POSITION_2:
            if action == 0:
                self._brake_robot2()
            elif action == 5:
                if self.agent_robot == AgentRobot.ROBOT2:
        ...
        elif robot_status == RLRobotFiniteState.PLACING_OBJECT:
        ...
        elif robot_status == RLRobotFiniteState.PLACING_OBJECT_UPPER:
            if action == 6:
        ...
        elif robot_status == RLRobotFiniteState.PLACING_OBJECT_LOWER:
            if action == 7:
        ...
        elif robot_status == RLRobotFiniteState.WAIT_FOR_FINISH:
            if action == 0:
                self._brake_robot2()   

            
        if self.shared_state["current_object_index"] >= len(self.object_joint_ids) and robot_status == RLRobotFiniteState.NAVIGATE_TO_PICKING_POSITION:
            if self.agent_robot == AgentRobot.ROBOT2:
                target_position_x_y = self.robot_2_origin_place_x_y
            elif self.agent_robot == AgentRobot.ROBOT3:
                target_position_x_y = self.robot_3_origin_place_x_y
            ...
            if self.agent_robot == AgentRobot.ROBOT2:
                ...
                dist_to_target = np.linalg.norm(robot_2_rover_pos - target_position_x_y)
            elif self.agent_robot == AgentRobot.ROBOT3:
                robot_3_rover_pos = self.data.xpos[self.robot_3_rover_id][:2]
                dist_to_target = np.linalg.norm(robot_3_rover_pos - target_position_x_y)
    ...
    def _back_car_robot2(self):
        ...
        if self.agent_robot == AgentRobot.ROBOT3:
            self.data.ctrl[
                SECOND_ROBOT_ACTION_SPACE_LENGTH :
                SECOND_ROBOT_ACTION_SPACE_LENGTH + 2
            ] = [-3, 0]
    ...
    def _predict_navigation_action_robot_2(self, target_position, direction, reach_threshold):
        ...
        if self.agent_robot == AgentRobot.ROBOT2:
            ...
            robot_3_rover_pos = self.data.xpos[self.robot_3_rover_id][:2]
            dist_to_target = np.linalg.norm(robot_3_rover_pos - target_position)
    ...
    def _get_navigation_obs(self):
        if self.agent_robot == AgentRobot.ROBOT2:
            robot_2_rover_id = self.robot_2_rover_id
        elif self.agent_robot == AgentRobot.ROBOT3:
            robot_2_rover_id = self.robot_3_rover_id
        
        robot2_pos = self.data.xpos[robot_2_rover_id][:2]  
        
        try:
            robot2_body_id = robot_2_rover_id
            robot2_vel = self.data.cvel[robot2_body_id][:2]  
        except:
            robot2_vel = np.zeros(2)
        
        robot2_quat = self.data.xquat[robot_2_rover_id]
        ...
        if self.agent_robot == AgentRobot.ROBOT2:
            target_position_x_y = self.robot_2_target_position_x_y
        elif self.agent_robot == AgentRobot.ROBOT3:
            target_position_x_y = self.robot_3_target_position_x_y

        if target_position_x_y is None:
            target_pos = robot2_pos.copy()  
        else:
            target_pos = np.array(target_position_x_y)
    ...
    def _get_obs(self):
        ...
        robot3_pos = self.data.xpos[self.robot_2_rover_id][:2]

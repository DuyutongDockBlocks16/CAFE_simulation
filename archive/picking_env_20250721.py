...
class SecondRobotPickingMuJoCoEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        # print gear of robot2:Joint1 to robot2:Joint5
        # joint1_gear = self.model.actuator_gear[joint1_actuator_id]
        # joint2_gear = self.model.actuator_gear[joint2_actuator_id]
        # joint3_gear = self.model.actuator_gear[joint3_actuator_id]
        # joint4_gear = self.model.actuator_gear[joint4_actuator_id]
        # joint5_gear = self.model.actuator_gear[joint5_actuator_id]
        # print(f"Joint1 gear: {joint1_gear}, Joint2 gear: {joint2_gear}, Joint3 gear: {joint3_gear}, Joint4 gear: {joint4_gear}, Joint5 gear: {joint5_gear}")

        # # 🎯 修正：使用正确的joint ID获取damping
        # robot1_joints = ["robot1:Joint1", "robot1:Joint2", "robot1:Joint3", "robot1:Joint4", "robot1:Joint5"]
        # robot1_dampings = []
        
        # for joint_name in robot1_joints:
        #     joint_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, joint_name)
        #     if joint_id >= 0:
        #         damping = self.model.dof_damping[joint_id]
        #         robot1_dampings.append(damping)
        #     else:
        #         robot1_dampings.append(0.0)  # 默认值
        
        # print(f"Robot1 Joint dampings: {robot1_dampings}")
        
        # 🎯 同样修正robot2的damping获取
        # robot2_joints = ["robot2:Joint1", "robot2:Joint2", "robot2:Joint3", "robot2:Joint4", "robot2:Joint5"]
        # robot2_dampings = []
        
        # for joint_name in robot2_joints:
        #     joint_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, joint_name)
        #     print(f"Joint name: {joint_name}, Joint ID: {joint_id}")
            
        #     if joint_id >= 0:
        #         # 🔥 修正：直接使用DOF地址，不需要DOF数量
        #         dof_adr = self.model.jnt_dofadr[joint_id]  # DOF的起始地址
                
        #         print(f"  DOF address: {dof_adr}")
                
        #         # 🔥 对于旋转关节，直接获取damping
        #         damping = self.model.dof_damping[dof_adr]
        #         robot2_dampings.append(damping)
        #         print(f"  Damping: {damping}")
                
        #     else:
        #         robot2_dampings.append(0.0)
        #         print(f"  Joint not found!")
        
        # print(f"Robot2 Joint dampings: {robot2_dampings}")
    ...
    def _calculate_reward(self):
        ...
        dropped = False
        # moved = False
        # if self.initial_object_height is not None and object_position[2] < 0.27:
        #     print("⚠️ Object dropped below initial height, applying penalty.")
        #     # print(f"Active position: {active_position}, Initial height: {self.initial_object_height}")
        #     total_reward += -self.reward_weights["dropped_penalty"]
        #     dropped = True
        ...
        distance_reward = max(self._calculate_distance_reward(alignment_current_center_distance), -100)
        total_reward += distance_reward
        self.previous_center_distance = alignment_current_center_distance
